...
async имеет конструктор подобный std::thread, так же принимает функцию и ее аргументы. Дополнительно первым аргументом может быть указано правило запуска потока - std::launch::async или std::launch::deferred. В первом случае новый поток запустится асинхронно, во втором же случае новый поток создан не будет, а результат начнет вычисляться в текущем потоке по первому запросу.
Если же указаны оба флага - поведение зависит от реализации, ни одного - аналогично.
